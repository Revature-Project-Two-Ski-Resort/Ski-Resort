    ...
    Object.keys(iterateObject).forEach(function (itemPrice) {
        if(print && typeof(iterateObject[itemPrice]) != "object" && typeof(iterateObject[itemPrice]) != "function") {
            if(document.getElementById(print_loc)) {
                ...
                document.getElementById(makeDiv).appendChild(makeP);

                document.getElementById(makeP).innerHTML += `${itemPrice}:${iterateObject[itemPrice]}`;
                //increase counter
                counter++;
            }
        }
        if(typeof(iterateObject[itemPrice]) == "object") {
            price += getTotalPrice(iterateObject[itemPrice], null, print, `data-level${counter}`);
        }
    });
    return price;
}
