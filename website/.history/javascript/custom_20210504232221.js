                    ...
                    if (employee_work[i]["Application ID"]) {
                        ...
                        if (employee_work[i]["Date Completed"] != null) {
                            ...
                            <li>
                                    ...
                                    <div class="row">
                                        <div class="col-7">
                                            <div class="card-body">
                                                ...
                                                <textarea oninput="block_deny(this, 'denier_${employee_work[i]["Work ID"]}');" class="card-text" cols="40" rows="5" maxlength="255">${my_desc}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="card-body">
                                ...
                                <button type="submit" id="approverbtn_${employee_work[i]["Work ID"]}" onclick="approve_app(${employee_work[i]["Urgency level"]}, ${employee_work[i]["Application ID"]}, ${employee_work[i]["Employee ID"]}, ${employee_work[i]["Work ID"]});" class="btn btn-outline-success">Approve Application
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                        <path d="M10.97 4.97a.235.235 0 0 0-.02.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05z"/>
                                    </svg>
                                </button>
                                <button type="submit" id="requestorbtn_${employee_work[i]["Work ID"]}" class="btn btn-outline-success">Request Additional Information
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                        <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
                                    </svg>
                                </button>
                                <button type="submit" id="denier_${employee_work[i]["Work ID"]}" disabled="disabled" class="btn btn-outline-danger">Deny Request
...
function approve_app(work_urgency, work_app, work_emp, work_id) {
    console.log("we get this far");
    if (work_id_holder) {
        let update_app = new Object();
        console.log("this is my workId: " + work_id_holder)
        update_app.urgency_level = work_urgency;
        update_app.description = document.getElementById(`app_desc_${app_id_holder}`).value;
        update_app.approval = true;
        
        let xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            let pre_load_ani = document.createElement("div");
            pre_load_ani.setAttribute("class", "loader");
            if(document.getElementById(`msg_loader${app_id_holder}`)) {
                document.getElementById(`msg_loader${app_id_holder}`).appendChild(pre_load_ani);
            }

            if (this.readyState == 4 && this.status == 200) {
                document.getElementById(`msg_loader${app_id_holder}`).innerHTML = "";
                ...
                for (let i = 0; i < app_elements.length; i++) {
                    if (app_elements[i]["Employee ID"]) {
                        ...
                        document.getElementById(`msg_loader${app_id_holder}`).innerHTML = `<h6>This approval is completed</h6>`;
                    }
                }
            } else if (this.readyState == 4 && this.status == 400) {
                document.getElementById(`msg_loader${app_id_holder}`).innerHTML = `<p>${this.responseText}</p>`
            }

        }

        url = `http://localhost:5000/users/${your_data["Employee ID"]}/work/${work_id_holder}`;
