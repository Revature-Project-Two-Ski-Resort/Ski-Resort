...
function addLodge(room, errormsg, roomId) {
...
function addroomelement() {
    ...
    document.getElementById("newroomspace").innerHTML += `
    <div id="nextroom${itemcounter}" class="form-group">
        <label for="inputAddress${itemcounter}">Rooms</label>
        <div class="form-row">
            <select onchange="addLodge(this.value, 'roomerror${itemcounter}', 'room${itemcounter}')" class="custom-select col-sm-9" id="mtn-bed${itemcounter}" name="mtn-bed${itemcounter}" aria-label="Choose a suite" required>
...
function setDateRange(DateElement) {
    if(document.getElementById(DateElement)) {
        let todays_date = new Date();
        let future_date = new Date();
        document.getElementById(DateElement).setAttribute("min", getDateFormat(todays_date));
        document.getElementById(DateElement).setAttribute("max", getDateFormat(future_date, 0, 0, 1));
    } else {
