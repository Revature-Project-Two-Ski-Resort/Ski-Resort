                    ...
                    if (employee_work[i]["Application ID"]) {
                        ...
                        <div class="card w-100">
                            <div class="card-body w-100">
                                <h4 class="card-title" id="work_app_${employee_work[i]["Application ID"]}">${employee_work[i]["Application ID"]}</h4>
                                <p class="card-text">Approval for application(#${employee_work[i]["Application ID"]})</p>
                                <h5 class="card-title">Description:</h5>
                                <p class="card-text">${employee_work[i]["Description"]}</p>
                            </div>
                        </div>`;
                    }
                }
...
function get_my_app(app_id) {
    if (app_id) {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (let i = 0; i < employee_work.length; i++) {
                    if (employee_work[i]["Application ID"]) {
                        ...
                        stats.setAttribute("class", "btn btn-submit");
                        let my_text = document.createTextNode(employee_work[i]["Application ID"]);
                        stats.appendChild(my_text);
                        document.getElementById("complete_approvals").innerHTML += `
                        <div class="card w-100">
                            <div class="card-body w-100">
                                <h4 class="card-title">${employee_work[i]["Application ID"]}</h4>
                                <p class="card-text">Approval for application(#${employee_work[i]["Application ID"]})</p>
                                <h5 class="card-title">Description:</h5>
                                <p class="card-text">${employee_work[i]["Description"]}</p>
                            </div>
        ...
        document.getElementById("complete_approvals").innerHTML += `<p>A valid application Id was not provided!</p>`
    }
}