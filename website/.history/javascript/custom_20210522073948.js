        ...
        if(group_book.lodging != null) {
            ...
            Object.keys(group_book.lodging).forEach(function (item) {
                if(typeof(group_book.lodging[item]) != "function") {
                    makeDiv.innerHTML += `
                        <div class="row m-3">
                            <h4 class="col-12">${group_book.lodging[item].name}</h4>
                            <p class="col-4">Suite: ${group_book.lodging[item].name}</p>
                            <p class="col-4">Price Per night: ${formatter.format(group_book.lodging[item].price)}</p>
                            <p class="col-12">Total Price: <b>${formatter.format(group_book.lodging.totalprice())}</b></p>
                        </div>`;
                }
            });
        }
