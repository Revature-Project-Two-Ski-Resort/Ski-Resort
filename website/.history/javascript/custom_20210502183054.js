...
function block_deny(blocker, the_denied) {
    console.log(blocker.value.length);
    if (blocker.value.length<20) {
        document.getElementById(the_denied).setAttribute("disabled", "disabled");
    } else {
        document.getElementById(the_denied).removeAttribute("disabled");
    }
}
...
document.getElementById("sub_app").onclick(function (e) {
    let is_valid = validate_my_form(e);
    console.log(is_valid);
});

function validate_my_form(e) {
    
    e.preventDefault();

    let form = document.getElementById("my_new_app");
    form.preventDefault();
    let inputs = form.getElementsByTagName("input"), input = null, select = null, not_pass = false;
    ...
    for(let i = 0, len = inputs.length; i < len; i++) {
        ...
        if(input.type == "date" && !input.checked) {
            not_pass = true;
        } 
        else if(input.type == "date" && input.checked) {
            not_pass = false;
            break;
        }
    }
