...
function calculate_refundable_amt(your_data) {
}
...
function get_refundable_amt(your_data, my_loader) {
    ...
    ajax.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            console.log(app_elements);
            console.log(app_elements.calculated_refund);
            if (app_elements.calculated_refund) {
                //Get data to calculate total refundable amount
                let money_left = 1000-app_elements.calculated_refund;
                let money_applying_for = 0;
                let percent_eligible = 0;
                if(document.getElementById("event_types").value == "") {
                    percent_eligible = 0.8
                } else if(document.getElementById("event_types").value == "") {
                ...
                if (money_left <= 0) {
                    ...
                    total_reimbursable = money_left-money_applying_for
                    document.getElementById(my_loader).innerHTML = `<p>Projected refund from this application is (${formatter.format(money_left)})</p>`
                }
            }
        } else if (this.readyState == 4 && this.status == 400) {
