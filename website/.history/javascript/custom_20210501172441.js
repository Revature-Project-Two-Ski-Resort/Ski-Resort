        ...
        xhttp.onreadystatechange = function () {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (let i = 0; i < employee_work.length; i++) {
                    if (employee_work[i]["First Name"]) {
                        //Create work lists
                    }
                }
            } else if (this.readyState == 4 && this.status == 400) {
                let app_response = JSON.parse(this.responseText);
                document.getElementById("loading_container").innerHTML = `<p>${employee_work}</p>`
            }

        }

        url = "http://localhost:5000/users/" + employee["Employee ID"]+"/work";
        xhttp.open("GET", url, true);
        xhttp.send();
    }
}