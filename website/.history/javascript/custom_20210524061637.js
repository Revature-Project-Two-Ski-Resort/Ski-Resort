...
function getAllRooms() {
    ...
    ajaxCaller(request_type, url, response_func, response_loc, load_loc)
}
//caller function: calls an ajax request
function getAllRooms_complete(status, response, response_loc, load_loc) {
    ...
    if(status == 200) {
        ...
        //load the response into the response_loc
        outputLodge(JsonHolder,"mtn_bed")
        popLodge(group_book.lodging);
...
//function to output the lodging options in the select
//print the lodging to a var
function outputLodge(lodgeObj, mySelect) {
    lodgeObj.forEach(element => {
        lodging[element.name] = new Object();
        lodging[element.name].id = element.id;
        lodging[element.name].name = element.name;
        lodging[element.name].price = element.price;
        lodging[element.name].viewtype = element.type;
        document.getElementById(mySelect).innerHTML += `<option>${element.name}</option>`
    });
    console.log(lodging);
}
...
function addroomelement(iterateObject, elemID) {
    ...
    let lodgeholder = "";
    Object.keys(lodging).forEach(element => {
        lodgeholder += `<option>${element.name}</option>`;
    });
    let newEl = `
...
(function() {
    ...
    window.addEventListener('load', function() {
        ...
        if(group_book.totalprice() > 20) {
            $("#cartItems").html(`
            <button class="btn btn-warning" onclick="startbook(group_book.lastcompleted);"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cart" viewBox="0 0 16 16">
