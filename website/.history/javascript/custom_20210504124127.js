            ...
            ajax.onreadystatechange = function () {
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (let i = 0; i < app_elements.length; i++) {
                        if (app_elements[i]["Employee ID"]) {
                            //Get application data
                            document.getElementById("sub_message").innerHTML = `<p>Total amount projected to be refunded(${formatter.format(app_elements["Total Refunded"])})</p>`;
                        }
                    }
                } else if (this.readyState == 4 && this.status == 400) {
...
function calculate_refundable_amt(my_loader) {
    ...
    if (cal_refund) {
        ...
        if (money_left <= 0) {
            ...
            document.getElementById(my_loader).innerHTML = `<p>Total projected refund so far(${formatter.format(cal_refund.calculated_refund)}) for ${new Date().getUTCFullYear()}</p>
