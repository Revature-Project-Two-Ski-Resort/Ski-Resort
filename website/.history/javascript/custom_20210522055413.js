...
//prints a recipt
function reciptController(myObj) {
    if(myObj.users != null) {
        let makeDiv = document.createElement("div");
        makeDiv.setAttribute("class", "row m-2");
        makeDiv.setAttribute("id", "Users");
        document.getElementById("confirm-data").appendChild(makeDiv);
        Object.keys(myObj.users).forEach(function (item) {
            if(typeof(myObj.users[item]) != "object" && typeof(myObj.users[item]) != "function") {
                Object.keys(myObj.users[item]).forEach(function (itemProps) {
                    if(typeof(myObj.users[item][itemProps]) != "object" && typeof(myObj.users[item][itemProps]) != "function") {
                        makeDiv.innerHTML += `${myObj.users[item]}`;
                    }
                });
            }
        });
