...
function addLodge(room, errormsg) {
    let lodgeCounter = 0;
    if(lodging[room]) {
        while(group_book["room"+lodgeCounter] != null) {
            lodgeCounter++;
        }
        group_book["room"+lodgeCounter] = lodging[room];
    } else {
        document.getElementById(errormsg).innerHTML = "<p>That room does not exist!</p>";
    }
