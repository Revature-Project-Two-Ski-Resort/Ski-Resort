...
function reciptController() {
    if(group_book.users != null) {
        ...
        Object.keys(group_book.users).forEach(function (item) {
            console.log(typeof(group_book.users[item]) != "object" && typeof(group_book.users[item]) != "function");
            if(typeof(group_book.users[item]) != "object" && typeof(group_book.users[item]) != "function") {
                makeDiv.innerHTML += `
                ...
                <div id="primaryuser" class="row m-3">
                    ...
                    <h6>Total Price: <span id="primaryuser-price">$0.00</span></h6>
                </div>
                <hr class="bg-dark" />
                    <h6>Total Price: <span id="primaryuser-price">${group_book.users[item].totalprice()}</span></h6>
                    <hr class="bg-dark" />`;
            }
        });
    }
