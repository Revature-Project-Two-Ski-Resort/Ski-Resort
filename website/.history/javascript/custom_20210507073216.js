            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (let i = 0; i < employee_work.length; i++) {
                    if (employee_work[i]["Application ID"]) {
                        ...
                        get_my_app(employee_work[i]["Application ID"], 40);
                        let today = new Date();
...
function final_app_approve(remove_btn, app_id, update_to_grade, my_id) {
    update_to_grade.setAttribute("disabled", "disabled");
    remove_btn.setAttribute("hidden", "hidden");
    update_data = new Object();
    update_data.final_grade = update_to_grade.value;
...
function get_my_app(app_id, emp_id) {
    if (app_id) {
        let xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            let pre_load_ani = document.createElement("div");
                ...
                for (let i = 0; i < app_elements.length; i++) {
                    if (app_elements[i]["Employee ID"]) {
                        ...
                        if (app_elements[i]["Final Grade"] != null) {
                            ...
                            <button type="submit" id="final_approve_${app_elements[i]["Application ID"]}" onclick="final_app_approve(this, app_id, update_to_grade, emp_id)" class="btn btn-outline-success">Approve Grade
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16">
                                    <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                    <path d="M10.97 4.97a.235.235 0 0 0-.02.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05z"/>
                                </svg>
