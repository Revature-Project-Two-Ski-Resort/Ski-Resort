...
function caller() {
    //do some logic to come up with the data you will be passing in
    ajaxCaller()
}
//caller function: calls an ajax request
function caller_complete(readyState, status, response) {
    //do some logic with the ajax data that was returned
}
function ajaxCaller(request_type, url, response_func, response_loc, jsonData) {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let i = 0; i < response_holder.length; i++) {
                if (response_holder[i].Email) {
                    ...
                    node.setAttribute("href", `${send_me}${response_holder[i]["Employee ID"]}`);
                    node.setAttribute("id", `help_email${i}`);
                    let textnode = document.createTextNode(response_holder[i].Email);
