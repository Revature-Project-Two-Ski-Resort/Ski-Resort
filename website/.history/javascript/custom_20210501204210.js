            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (let i = 0; i < app_elements.length; i++) {
                    if (app_elements[i]["Employee ID"]) {
                        ...
                        document.getElementById(`app_cost_${app_id}`).innerHTML = `${app_elements[i]["Total Cost"]}`;
                        document.getElementById(`app_desc_${app_id}`).innerHTML = `${app_elements[i]["Description"]}`;
                        document.getElementById(`app_edate_${app_id}`).innerHTML = `${app_elements[i]["Event Date"]}`;
                        document.getElementById(`app_etype_${app_id}`).innerHTML = `${app_elements[i]["Event Type"]}`;
                        document.getElementById(`app_loc_${app_id}`).innerHTML = `${app_elements[i]["Address"]} ${app_elements[i]["Second Address"]}  ${app_elements[i]["City"]},  ${app_elements[i]["State"]}`;
