    ...
    if (search_query) {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                console.log("load emp by email:"+employees);
    ...
    if (my_user_id) {
        ...
        xhttp.onreadystatechange = function () {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                your_data = employee;
                console.log("preload info function:"+employee);
    ...
    if (my_user_id) {
        ...
        xhttp.onreadystatechange = function () {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                let employee = JSON.parse(this.responseText);
                your_data = employee;
                console.log("preload profile function:"+employee);
                for (let i = 0; i < employee.length; i++) {
                    if (employee[i]["First Name"]) {
                        //Pre-fill in the form data
    ...
    if (employee) {
        ...
        xhttp.onreadystatechange = function () {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                console.log("Load work function:"+employee_work);
                    ...
                    if (employee_work[i]["Application ID"]) {
                        ...
                        if (employee_work[i]["Date Completed"] != null) {
                            ...
                            <li id="approve_item_${employee_work[i]["Work ID"]}">
                                ...
                                <span>
                                    <button type="submit" disabled='disabled' ${disable_dept} id="requestorbtn_${employee_work[i]["Work ID"]}_dept" onclick="request_add_info(${employee_work[i]["Application ID"]}, 'textarea_${employee_work[i]["Application ID"]}', 'dept_approval', this);" class="btn btn-outline-success">Request Additional Information(depthead)
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                            <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
                                        </svg>
                                    </button>
                                    <button type="submit" disabled='disabled' ${disable_sup} id="requestorbtn_${employee_work[i]["Work ID"]}_sup" onclick="request_add_info(${employee_work[i]["Application ID"]}, 'textarea_${employee_work[i]["Application ID"]}', 'sup_approval', this);" class="btn btn-outline-success">Request Additional Information(supervisor)
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                            <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
                                        </svg>
                                    </button>
                                    <button type="submit" disabled='disabled' id="requestorbtn_${employee_work[i]["Work ID"]}" onclick="request_add_info(${employee_work[i]["Application ID"]}, 'textarea_${employee_work[i]["Application ID"]}', 'emp', this);" class="btn btn-outline-success">Request Additional Information(employee)
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                            <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
                                        </svg>
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(response_holder.length > 0) {
                for (let i = 0; i < response_holder.length; i++) {
                    document.getElementById("complete_add_info").innerHTML = += `
                    <li id="complete_item_${response_holder[i]["Application ID"]}">
                        <div class="card">
                            <div class="row">
                                <div class="col-7">
                                    <div class="card-body">
                                        <div id="add_msg_loader"></div>
                                        <h4 class="card-title" id="work_app_${response_holder[i]["Application ID"]}"></h4>
                                        <p class="card-text">Approval for applicationID(#${response_holder[i]["Application ID"]}) ${urgent_desc}</p>
                                        <h5 class="card-title">Date submitted:</h5>
                                        <p class="card-text">${response_holder[i]["Date Received"]}</p>
                                        <h5 class="card-title">Date Approved:</h5>
                                        <p class="card-text">${my_date}</p>
                                        <h5 class="card-title">Reason:</h5>
                                        <textarea id="textarea_${response_holder[i]["Application ID"]}" oninput="block_deny(this, 'requestorbtn_${employee_work[i]["Work ID"]}_dept');block_deny(this, 'requestorbtn_${employee_work[i]["Work ID"]}_sup');block_deny(this, 'requestorbtn_${employee_work[i]["Work ID"]}');${dis_req_func}" class="card-text" cols=40 rows=5 maxlength=255>${my_desc}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <button type="submit" id="approverbtn_${response_holder[i]["Work ID"]}" onclick="approve_app(${employee_work[i]["Urgency level"]}, ${employee_work[i]["Application ID"]}, ${employee_work[i]["Employee ID"]}, ${employee_work[i]["Work ID"]}, true);" class="btn btn-outline-success">Approve Application
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16">
                                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                <path d="M10.97 4.97a.235.235 0 0 0-.02.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05z"/>
                            </svg>
    ...
    if (work_id) {
        ...
        xhttp.onreadystatechange = function () {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                console.log("approval function:"+app_elements);
...
function your_level(your_data) {
    ...
    ajax.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            console.log("Your level function:"+your_data[0]);
            for (let i = 0; i < app_elements.length; i++) {
...
if(document.getElementById("sub_app")) {
    document.getElementById("sub_app").addEventListener("click", function(e){
        ...
        if(is_valid) {
            ...
            console.log("Your form data in the button listener sub app:"+formData);
...
function calculate_refundable_amt(my_loader) {
    ...
    console.log("calculate refundable amt function:"+cal_refund);
    if (cal_refund) {
        let money_left = 1000;
