    ...
    $.each(formObject,function(index,item){
        $.each(item,function(key,value){
            if(key.indexOf(".") == -1) {
                ...
                outputObject[mainkey][subkey] = services[key];
                if(typeof(value) == "boolean") {
                    outputObject[mainkey][subkey].quantity = 1;
                    outputObject[mainkey][subkey].isselected = value;
                } else {
                    outputObject[mainkey][subkey].quantity = value;
                }
                if(value == "" || value == false) {
                    delete outputObject[mainkey][subkey];
                }
            }
        });
    });
...
function preloadService(formObject) {
    $.each(formObject,function(index,item){
        if(document.getElementById(index)) {
            if(typeof(formObject[index]) == 'boolean') {
                $('#'+index).prop('checked', true);
            } else {
                document.getElementById(index).value = formObject[index];
            }
        }
    });
}
function preloadTrails() {
    console.log("hi");
}
function preloadCamp() {
    console.log("hello");
}
//camping save
