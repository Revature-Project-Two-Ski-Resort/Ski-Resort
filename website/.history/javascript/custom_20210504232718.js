...
function approve_app(work_urgency, work_app, work_emp, work_id) {
    ...
    if (work_id) {
        ...
        update_app.description = document.getElementById(`app_desc_${work_app}`).value;
        ...
        xhttp.onreadystatechange = function () {
            let pre_load_ani = document.createElement("div");
            pre_load_ani.setAttribute("class", "loader");
            if(document.getElementById(`msg_loader${work_app}`)) {
                document.getElementById(`msg_loader${work_app}`).appendChild(pre_load_ani);
            }

            if (this.readyState == 4 && this.status == 200) {
                document.getElementById(`msg_loader${work_app}`).innerHTML = "";
                ...
                for (let i = 0; i < app_elements.length; i++) {
                    if (app_elements[i]["Employee ID"]) {
                        ...
                        document.getElementById(`msg_loader${work_app}`).innerHTML = `<h6>This approval is completed</h6>`;
                    }
                }
            } else if (this.readyState == 4 && this.status == 400) {
                document.getElementById(`msg_loader${work_app}`).innerHTML = `<p>${this.responseText}</p>`
            }

        }

        url = `http://localhost:5000/users/${your_data["Employee ID"]}/work/${work_id}`;
